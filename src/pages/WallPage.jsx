...
function formatDeliveredDate(timestamp, locale) {
...
function formatShortDate(timestamp, locale) {
  if (!timestamp?.toDate) return "";
  return timestamp.toDate().toLocaleDateString(locale === "en" ? "en-US" : "zh-TW", {
    year: "numeric",
    month: "numeric",
...
function getJournalExcerpt(entry, locale) {
  ...
  if (parts.length === 0) {
    return locale === "en"
      ? "The scene you kept imagining has now become part of reality."
      : "你曾經反覆感受到的畫面，現在成為現實的一部分。";
  }
...
function WallStats({ deliveredCount, activeCount, projectionDays }) {
  ...
  const items = [
    { value: deliveredCount, label: locale === "en" ? "Fulfilled" : "已實現" },
    { value: activeCount, label: locale === "en" ? "In transit" : "運送中" },
    { value: projectionDays, label: locale === "en" ? "Projection days" : "投射天數" },
  ];
  ...
  return (
    <section className="paper-card border-dashed px-6 py-16 text-center">
      ...
      <p className="mx-auto mt-5 max-w-[18rem] text-[1.02rem] leading-[2] text-[color:var(--ink-soft)]">
        {locale === "en"
          ? "Your order has already been sent. The universe is packing it. When the wish lands, it will stay here."
          : <>你的目標已送出，<br />宇宙正在打包。<br />當願望落地的那一刻，<br />它會永遠留在這裡。</>}
      </p>
    ...
    <article className="paper-card overflow-hidden">
      <div className="relative h-48 overflow-hidden rounded-b-none rounded-t-[1.6rem]" style={{ background: theme.background }}>
        ...
        <div className="absolute right-4 top-4 flex h-[4.4rem] w-[4.4rem] flex-col items-center justify-center rounded-full border border-[rgba(181,120,58,0.6)] bg-[rgba(245,239,230,0.96)] text-[color:var(--gold)] shadow-[0_12px_24px_rgba(46,35,24,0.12)]">
          ...
          <span className="mt-1 text-[0.72rem] tracking-[0.16em]">{locale === "en" ? "Ful" : "已"}</span>
          <span className="text-[0.72rem] tracking-[0.16em]">{locale === "en" ? "filled" : "送達"}</span>
        </div>
      </div>
      ...
      <div className="px-5 py-5">
        <div className="flex items-center justify-between gap-4">
          ...
          <span className="text-[0.9rem] tracking-[0.08em] text-[color:var(--ink-soft)]">
            {locale === "en" ? `Fulfilled in ${journeyDays} days` : `${journeyDays} 天後實現`}
          </span>
        </div>
        ...
        {latestEntry ? (
          <blockquote className="mt-5 border-l border-[rgba(181,120,58,0.28)] pl-4 text-[1.04rem] italic leading-[1.95] text-[color:var(--ink-soft)]">
            {getJournalExcerpt(latestEntry, locale)}
          </blockquote>
        ) : null}
        ...
        <div className="mt-6 border-t border-[rgba(181,120,58,0.12)] pt-5">
          <div className="flex items-center justify-between gap-4">
            <p className="text-[0.9rem] tracking-[0.12em] text-[color:var(--ink-faint)]">
              {formatDeliveredDate(order.deliveredAt, locale) || `${formatOrderMonth(order.createdAt)} ${locale === "en" ? "fulfilled" : "實現"}`}
            </p>
            <button
              type="button"
              onClick={onOpen}
    ...
    <div className="space-y-5">
      {entries.length === 0 ? (
        <p className="text-sm leading-7 text-[color:var(--ink-soft)]">
          {locale === "en"
            ? "There is no projection journal yet, but this wish was still received."
        ...
        entries.map((entry, index) => {
          ...
          const label = isLast
            ...
            : (locale === "en" ? `Projection ${index + 1}` : `${index === 0 ? "第 1 次投射" : `第 ${index + 1} 次投射`}`);
      ...
      <section className="paper-card overflow-hidden px-0 py-0">
        ...
        {latestEntry ? (
          <div className="border-t border-[rgba(181,120,58,0.1)] bg-[rgba(250,246,240,0.55)] px-6 py-5">
            <p className="text-sm leading-7 text-[color:var(--ink-soft)]">
              {locale === "en" ? "Latest felt scene" : "最新留下的感受"}：{getJournalExcerpt(latestEntry, locale)}
            </p>
          </div>
        ) : null}
      </section>
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h1 className="section-title mt-2 text-[2.7rem] leading-[1.1]">
            {locale === "en" ? <>Fulfilled<br />Manifestations</> : <>已實現的<br />顯化</>}
          </h1>
        </div>
        <OrdersModeToggle
          mode="delivered"
